...
#############################################################################
#                                                                           #
# Name: col_gpfs_mmdiag--stats.ksh                                          #
# Path: N/A                                                                 #
# Host(s): N/A                                                              #
# Info: Script to collect pagepool utilization                              #
#                                                                           #
...
DATE=$(date +"%Y%m%d")
TIME=$(date +"%H%M%S")
GPFSPPTXT=gpfs_mmdiag--stats_${HOST}_${DATE}_${TIME}.out
...
function check_mmdiag {
    ...
    type mmdiag > /dev/null
    RC=$?
    if (( $RC != 0 ))
    then
        printf "mmdiag command not available!!!\n"
        exit -1
    fi
...
function get_mmdiag_stats {
    ...
    echo "### BEGIN DATA COLLECTION $(date +"%Y/%m/%d") $(date +"%H:%M:%S") ###" >> ${COL_DIR}/${GPFSPPTXT}
    mmdiag --stats >> ${COL_DIR}/${GPFSPPTXT} 
    echo "### END DATA COLLECTION $(date +"%Y/%m/%d") $(date +"%H:%M:%S") ###" >> ${COL_DIR}/${GPFSPPTXT}
